...
for la = 0,n_elements(currLatSel) -1 do begin
    for ml = 0,n_elements(currMLTSel) -1 do begin
        lat = currLatSel[la]
        currNM = currMLTSel[ml]
        ...
        if ( currPotMltInds[0] eq -1 ) then begin
            ...
            if ( currPotMlatInds[0] eq -1 ) then begin
                ...
                print, potsArr[ la, ml ], lat, currMlt, currNM, stereoCoords
            endelse
...
;        /overplot, xstyle=4, ystyle=4, noclip=0, thick = 2., /follow

contour, potsArr, strLatArr, strMltArr, $
        /overplot, xstyle=4, ystyle=4, noclip=0, thick = 2., $
        levels=cntrMaxVal-(potScale[0]-cntrMaxVal)*findgen(n_levels+1.)/float(n_levels), /follow
